...
class AirfoilGeometrySampler:
	...
	def create_mesh(self, x_camber: np.ndarray, upper_curve: np.ndarray, lower_curve: np.ndarray) -> None:

		my_mesh = Mesh()

		top_left = Entity.Point([-1, 0.5, 0])
		top_right = Entity.Point([2, 0.5, 0])
		bottom_left = Entity.Point([-1, -0.5, 0])
		bottom_right = Entity.Point([2, -0.5, 0])
		...
		domain_outer_boundary = [top_boundary, right_boundary, bottom_boundary, left_boundary]
		...
		subprocess.run(["gmsh", "my_mesh.geo", "-2", "-o", "newer_mesh.stl", "-format", "stl", "-save_all"])
