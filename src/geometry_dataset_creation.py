...
class AirfoilGeometrySampler:
	...
	def _compute_airfoil_geometry(self, c1: float, c2: float, c3: float, c4: float, X_T: float, T: float, rho_bar: float, beta_bar: float) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:

		rho = rho_bar * (T / X_T) ** 2
		beta = beta_bar * np.arctan(T / (1 - X_T))

		# Camber line
		x_camber = 3 * c1 * self.n_points * (1 - self.n_points) ** 2 + 3 * c2 * (1 - self.n_points) * self.n_points ** 2 + self.n_points ** 3
		y_camber = 3 * c3 * self.n_points * (1 - self.n_points) ** 2 + 3 * c4 * (1 - self.n_points) * self.n_points ** 2

		# Assemble linear system to solve for thickness parameters
		A = np.array([[np.sqrt(X_T), X_T, X_T ** 2, X_T ** 3, X_T ** 4], [0.5 / np.sqrt(X_T), 1, 2 * X_T, 3 * X_T ** 2, 4 * X_T ** 3], [0.25, 0.5, 1, 1.5, 2], [1, 0, 0, 0, 0], [1, 1, 1, 1, 1]])
		R = np.array([T, 0, -np.tan(beta / 2), np.sqrt(2 * rho), 0])

		t = np.linalg.solve(A, R)

		thickness = t[0] * np.sqrt(x_camber) + t[1] * x_camber + t[2] * x_camber ** 2 + t[
			3] * x_camber ** 3 + t[4] * x_camber ** 4
	...
	def sample_airfoil_geometry(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
		...
		x_camber, upper_curve, lower_curve = self._compute_airfoil_geometry(c1=c1, c2=c2, c3=c3, c4=c4, X_T=X_T, T=T, rho_bar=rho_bar, beta_bar=beta_bar)
		return x_camber, upper_curve, lower_curve

	def plot_airfoil(self, x_camber, upper_curve, lower_curve, filename: Optional[str] = None):
